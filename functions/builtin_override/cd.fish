function __cd_explicit_path
  test -z $argv && echo $HOME || echo (realpath $argv)
end
...
function cd --description "Wrapper function for change directory"
  ...
  if set -q _flag_list        # Using list option
    ...
    test (count $argv) -gt 0 && echo "WARN: Change directory to index $_flag_index, ignore other arguments"
    set cd_destination $cd_history[$_flag_index]
  else # default case
    if test "$argv" = '-'
      set cd_destination $cd_prev_path
    else
      set cd_destination (__cd_explicit_path "$argv")
    end
  end

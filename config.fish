if status is-interactive
    ...
    set -g singularis_theme_path        $HOME/.config/fish/themes/mocha.fish
    ...
    abbr -a chmox   'sudo chmod +x'
    abbr -a rmfc    'rm ~/.config/fish/fish_variables'
    ...
    # Singularis default themes

    # Singularis addons
    set -l SINGULARIS  $HOME/.config/fish/functions/singularis
    if test -d $SINGULARIS
        for f in $SINGULARIS/*.fish
            source $f
        end
    end

    # Override fish functions
    set -l OVERRIDE_PATH  $HOME/.config/fish/functions/override
    if test -d $OVERRIDE_PATH
        for f in $OVERRIDE_PATH/*.fish
            source $f
        end
    end
